...
Both test targets declare `path: "Tests"` with an explicit `sources: [...]` and
`resources: [.copy("Fixtures")]`, because the fixture directory is shared, not per-target
and a resource path may not escape its target's directory.
...
| File | Responsibility |
|---|---|
| `FrugaRelayCore.swift` | `FrugaRelayVersion.shell` (the pinned CDN shell version, a compile-time constant bumped only by a release task), `FrugaRelayVersion.protocolVersion` (the bridge protocol version `ready` is checked against) and `FrugaRelayVersion.shellURL` — **the one place the shell URL is built**, and never from partner input |
| `FrugaNativeMessage.swift` | Shell → native: the eight `case`s, their payloads, the `TokenRequiredPayload.Reason` / `ErrorPayload.Code` / `LogPayload.Level` enums, and `decode(_:) throws`. The trust boundary: an unknown `type`, a missing field or an out-of-range enum throws. `log.data` is `unknown` on the TS side, so it is carried as re-serialised JSON bytes through a private `JSONValue` and never inspected |
...
| `FrugaShellSession.swift` | `@MainActor` session and the `@MainActor protocol FrugaShellTransport { func reload(); func send(_ json: Data) }`. Remembers `init` and replays it on every `shellDidLoad()`, decodes inbound JSON and reports what does not parse through `onDropped` (its raw `type` and nothing else), raises `PROCESS_TERMINATED` then reloads, reports `VERSION_MISMATCH` for a `ready` whose `protocolVersion` differs, fires `onClose` on an inbound `.close` (no dedup, no reply), and owns the `back` / `backResult` and `getBalance` / `balance` requests with their timeouts (an inbound `error` also fails a pending `getBalance`, rather than letting it wait out the timeout). Also `struct FrugaBalance` and the `<redacted>` marker (parity with Android's `FrugaShellSession.REDACTED`) |
...
| `FrugaLogger.swift` | `public protocol FrugaLogger { func log(level:event:data:) }`, the `data: [:]` default-argument overload Swift cannot put on the requirement itself, and `error(_ error: FrugaError)` — the shared `error` event shape, same name and keys as Android's `FrugaLogger.error`. Also `public func redactSecrets(_:)`, which replaces any `token=` / `partnerKey=` query value in untrusted text with `<redacted>` |
| `FrugaExternalURLRule.swift` | The one gate on what may leave the SDK at all: `http`/`https`, case-insensitive. Everything else is dropped and logged as `navigation.blocked` by the caller |

### `Sources/FrugaRelay` — UIKit/WebKit

| File | Responsibility |
|---|---|
| `FrugaRelay.swift` | The partner facade (`enum FrugaRelay`, no instances): `configure`, `open(from:onError:)`, `close()`, `getBalance(timeout:)`, the stored `config`, a **weak** reference to the presented controller plus a separate **weak** `liveScreen` (any controller registered via `registerHostedScreen`/`unregisterHostedScreen`, whether presented by `open` or hosted directly by the partner — `getBalance` answers through `liveScreen`, Android parity), the partner-settable `logger` (default `OSLogLogger`, in this file: `os.Logger`, subsystem `uk.co.fruga.relay`, category `FrugaRelay`, `debug` lines dropped unless `options.debug`), and the process-wide `NWPathMonitor` started by `configure`. Its path updates set `isOnline` and forward a `network` message to the presented screen on the main actor |
| `FrugaRelayViewController.swift` | The screen: owns the `WKWebView`, the `fruga` script message handler (through a `ScriptMessageProxy`, so the WebView's content controller does not retain the controller), the `FrugaShellSession`, the `FrugaTokenCoordinator`, the `init`-payload composition and safe-area measurement, the foreground observer, `SFSafariViewController` presentation, the swipe-down `UIAdaptivePresentationControllerDelegate`, and the logger wiring: every `FrugaError` that reaches the partner also reaches `FrugaRelay.logger.error(_:)`, an inbound `log` message is forwarded to the sink, an inbound `error` takes the same path as a native one (sink **and** `onError`, once each) (data flattened to `parent.child` pairs with `partnerKey` and token-shaped keys dropped at every depth, arrays dropped), `session.onDropped` becomes a `warn` `bridge.dropped` line carrying only the type, `session.onClose` is set after `super.init` and calls `performDismiss()` only while `presentingViewController != nil` (a late or repeat close on an already-dismissed screen is a no-op, same guard as `presentationControllerShouldDismiss`), and a blocked non-http(s) scheme becomes a `warn` `navigation.blocked` line carrying only the scheme |
...
returns on anything else. `FrugaShellSession.receive` drops anything
`FrugaNativeMessage.decode` throws on — malformed JSON, an unknown `type`, an
out-of-range enum — and reports the drop through `onDropped` with the body's raw `type`
when it has one (`nil` otherwise), which the controller logs as `bridge.dropped`.
Nothing on the inbound path throws or traps. A decoded message the session does not act
on (`error`, `log`) still reaches `onMessage`, and so do the ones it does: a `ready`
whose `protocolVersion` differs from `FrugaRelayVersion.protocolVersion` is **reported,
not dropped** (`VERSION_MISMATCH`, `recoverable: false`), and a `balance` resolves every
pending `requestBalance`. A malformed `openExternal.url` is dropped by `URL(string:)`.
...
| `viewDidDisappear` | → `relayDidDisappear(isDismissing: isBeingDismissed \|\| isMovingFromParent)`. When dismissing or popping: `FrugaRelay.unregisterHostedScreen(self)` (a dismissed or popped screen stops answering `getBalance()` even if the partner still retains it — this runs here, not `deinit`, because a weak `liveScreen` read during `deinit` is already `nil`) then `Task { await coordinator.cancel() }`. A covered-but-still-presented screen (neither flag true) keeps its in-flight request and stays registered |
| `viewDidAppear` | `FrugaRelay.registerHostedScreen(self)` — re-registers on reappear (e.g. a screen returning to view after another was popped in front of it) |
...
The widget's header ✕ forwards as a bridge `close` message, never answered and possibly
repeated. `session.onClose` — set on `FrugaRelayViewController` after `super.init`, next to
`onTokenRequired` and `onOpenExternal` — calls `performDismiss()` only while
`self.presentingViewController != nil`, the same late-close guard `back` uses, so a
repeat `close` or one arriving after the screen is already dismissed is a no-op rather
than walking up to a presenter that is no longer this screen's.
...
Both paths go through `FrugaRelayViewController.openIfAllowed`, which logs the drop as a
`warn` `navigation.blocked` line carrying only `["scheme": ...]` — never the URL, which
can carry personal data (parity with Android's `openIfAllowed`).
...
| `NWPathMonitor` | Runs on its own `uk.co.fruga.relay.network` queue and hops into `Task { @MainActor }` before touching `isOnline` or the presented screen |
...
| `relayDidDisappear(isDismissing:)` | `FrugaRelayViewController` | Called with `isBeingDismissed \|\| isMovingFromParent`, only set under a real presentation or push; this is the seam `viewDidDisappear` calls so cancel-on-close is testable. When dismissing, it unregisters the screen from `FrugaRelay` before cancelling the provider task |
...
### Core — `Tests/FrugaRelayCoreTests` (8 files, Linux and macOS)

| File | Covers |
|---|---|
| `FrugaNativeMessageTests` | All eight shell → native types decode from `*.valid.json`; every `*.invalid.json` throws; `UNSUPPORTED_ENGINE`'s raw value and literal decode; `Equatable` |
...
| `FrugaShellSessionTests` | `init` replay on every load and across a termination, `PROCESS_TERMINATED` un-deduped, malformed input dropped and reported through `onDropped` with its raw type, `ready` protocol-version match and mismatch, the full back request/result/timeout/repeat/unsolicited matrix, `requestBalance` send/resolve/timeout/two-in-flight, `openExternal` and `tokenRequired` routing, `lastSent` |
| `FrugaLoggerTests` | `error(_:)` maps a `FrugaError` onto the `error` event and its string data, every `ErrorPayload.Code` produces one, and the `data`-less `log` overload defaults to `[:]` |
...
### UIKit — `Tests/FrugaRelayTests` (11 files, macOS/simulator only)
...
| `FrugaRelayLoggerTests` | `FrugaRelay.logger` has a default, is replaceable, and `reset()` restores the default |
| `FrugaRelayLoggingTests` | The controller's wiring: an error reaching `onError` also reaches the sink, an inbound `log` is forwarded (with `token=` / `partnerKey=` query values redacted), a dropped message becomes `bridge.dropped`, a blocked scheme becomes `navigation.blocked` |
| `FrugaRelayGetBalanceTests` | `getBalance()` fails immediately with `BRIDGE_TIMEOUT` when no screen is live, and resolves from any live screen's session (presented via `open` or hosted directly) otherwise |
| `FrugaRelayHostedScreenTests` | `FrugaRelay.getBalance()` answers for a `FrugaRelayViewController` the partner presents and hosts itself (not via `FrugaRelay.open`), fails again once that controller is deallocated, an older hosted screen dropping does not unregister a newer one, `viewDidDisappear` unregisters the screen when dismissing/popping but keeps it registered otherwise, and `viewDidAppear` re-registers it |
| `FrugaRelayOpenDefaultTests` | `open(from:)` compiles and presents without an `onError` argument, relying on its `{ _ in }` default |
| `FrugaRelayShellErrorTests` | A shell-sent `error` message reaches the partner's `onError` closure (not only the logger) with the fixture's code, message and `recoverable` |
...
and drive the session directly. The exceptions are the seven whose controller actually
loads the shell: `FrugaRelayViewControllerTests.testWebViewLoadsTheCdnShellUrl`,
`testOpenPresentsAsPageSheetWithSwipeBackDelegate`, `testCloseDismissesThePresentedController`,
`testSwipeBackDelegateIsWiredByTheControllerItself` and `testDoubleOpenPresentsOnce`, plus
`FrugaRelayGetBalanceTests.testGetBalanceWithScreenPresentedForwardsToSessionAndSucceeds` and
`FrugaRelayOpenDefaultTests.testOpenCompilesAndPresentsWithoutAnOnErrorArgument`: those start
the **real** load against the CDN, so the macOS job does make network calls. Every other test
either sets `loadsShellAutomatically = false`, or never reaches `viewDidLoad` at all: a test
that calls `FrugaRelay.open` but hits an early guard (before `configure`, or offline) never
creates a controller, and a test like `testOpenExternalWithNonHttpSchemeIsDroppedNotOpenedExternally`
builds a controller directly but never mounts or presents it, so `viewDidLoad` never runs.
What none of the seven do is *wait* on the shell — no
assertion depends on the shell's JS, a rendered widget or a bridge round trip. That is the
deliberate contrast with Android, whose instrumented suite does wait on the live shell.
...
| `ios` | `macos-15`, 30 min | Picks the first available iPhone simulator from `xcrun simctl list devices available --json`, falling back to `xcodebuild -downloadPlatform iOS` and failing loudly if there still is none; then two `xcodebuild -scheme FrugaRelay -destination "id=$DEST_ID" -test-timeouts-enabled YES -default-test-execution-time-allowance 120 test` steps with `CODE_SIGNING_ALLOWED: NO`. `-only-testing:FrugaRelayTests` (WKWebView) runs plain, without Thread Sanitizer: bringing up a `WKWebView` in a host-less xctest process blocks the main actor for seconds even uninstrumented, and under TSan on the 3-core runner that stall exceeded the 120 s allowance, landing on a different test each attempt. `-only-testing:FrugaRelayCoreTests` runs second with `-enableThreadSanitizer YES` — it has no WebKit bring-up, so TSan stays well inside the allowance there, and it's the dynamic check that bridge-adjacent state main-actor isolation is actually respected, not just statically declared. Restoring TSan on `FrugaRelayTests` is tracked by issue #79 and gated on issue #152 (a `FrugaShellTransport` seam that lets a WKWebView-bearing test avoid the real bring-up) |
...
- **An unknown `tokenRequired.reason` still requests no token** — the message fails to
  decode, so `receive` drops it; it is now at least reported as `bridge.dropped` rather
  than silently. The rest of [#129](https://github.com/FrugaInsurance/sdk/issues/129)
  (the `ready.protocolVersion` check and the dropped-message log) has landed.
