...
| `FrugaShellSession.swift` | `@MainActor` session and the `@MainActor protocol FrugaShellTransport { func reload(); func send(_ json: Data) }`. Remembers `init` and replays it on every `shellDidLoad()`, decodes inbound JSON and reports what does not parse through `onDropped` (its raw `type` and nothing else), raises `PROCESS_TERMINATED` then reloads, reports `VERSION_MISMATCH` for a `ready` whose `protocolVersion` differs, and owns the `back` / `backResult` and `getBalance` / `balance` requests with their timeouts (an inbound `error` also fails a pending `getBalance`, rather than letting it wait out the timeout). Also `struct FrugaBalance` and the `<redacted>` marker (parity with Android's `FrugaShellSession.REDACTED`) |
